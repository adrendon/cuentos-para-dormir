...
async function hydrateDownloadedBook(book: Book): Promise<Book> {
  ...
  try {
    ...
    next = {
      ...
      author: parsed.author || next.author,
      illustrator: parsed.illustrator,
      description: parsed.description,
    };
  } catch {}
  try {
    const info: BookAdditionalInfo = JSON.parse(
      await FileSystem.readAsStringAsync(`${bookLocalPath}/AdditionalInfo.json`)
    );
    next = {
      ...next,
...
export function useBooks() {
    ...
    try {
      ...
      for (const entry of bookCatalog) {
        ...
        let base: Book = {
          ...
          commonPages: [],
          hasVoicework: false,
          isRead: readSet.has(entry.folderName),
          isFavorite: favSet.has(entry.folderName),
          isDownloaded: isAvailable,
          isEmbedded: entry.embedded,
          sizeMB: entry.sizeMB,
        };
  ...
  const markBookAsDownloaded = useCallback(
    (bookId: string) => {
  ...
  const deleteBook = useCallback(
    async (bookId: string) => {
      ...
      setBooks((prev) =>
        prev.map((b) => (b.id === bookId ? { ...b, isDownloaded: false, hasVoicework: false } : b))
      );
    },
    [books]
  );
  ...
  const filteredBooks = useMemo(
    () => getFilteredBooks(books, searchQuery, filters),
