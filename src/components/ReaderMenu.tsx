...
const TEXT_SIZES = [14, 18, 22, 26] as const;
const nearestIndex = (size: number) =>
  TEXT_SIZES.reduce(
    (best, value, index) =>
      Math.abs(value - size) < Math.abs(TEXT_SIZES[best] - size) ? index : best,
    0
  );
export function ReaderMenu({
  ...
  if (showTextSize) {
    ...
    const selectedIndex = nearestIndex(textSize);
    const closeSize = 54 * scale;
    return (
      <Animated.View style={[StyleSheet.absoluteFill, styles.overlay, { opacity: entrance }]}>
        <TouchableOpacity
          style={[
            styles.closeButton,
            {
              top: 18 * verticalScale,
              left: 66 * horizontalScale,
              width: closeSize,
              height: closeSize,
          ...
          onPress={onClose}
          accessibilityLabel="Cerrar tamaño del texto"
        >
          <Image
            source={require('../assets/ui/ic_close.png')}
            style={{ width: closeSize * 0.56, height: closeSize * 0.56 }}
            resizeMode="contain"
          />
        </TouchableOpacity>
        <Animated.View
              ...
              transform: [
                {
                  translateY: entrance.interpolate({
                    inputRange: [0, 1],
                    outputRange: [18 * verticalScale, 0],
                  }),
                },
        ...
        >
          <View style={[styles.sizeSliderRow, { marginBottom: 20 * verticalScale }]}>
            ...
            <Slider
              style={{
                width: Math.min(previewWidth * 0.72, 430 * horizontalScale),
              ...
              step={1}
              value={selectedIndex}
              onValueChange={(value) => onTextSizeChange(TEXT_SIZES[Math.round(value)])}
          ...
          <Text
            style={[
              styles.previewText,
              { fontSize: textSize * scale, lineHeight: textSize * scale * 1.36 },
            ]}
          >
            A primera vista era como cualquier otro gato - con orejas, bigotes y una larga cola.
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
            ...
            transform: [
              {
                translateY: entrance.interpolate({
                  inputRange: [0, 1],
                  outputRange: [-12 * verticalScale, 0],
                }),
              },
              {
                translateX: entrance.interpolate({
                  inputRange: [0, 1],
                  outputRange: [10 * horizontalScale, 0],
                }),
              },
      ...
      >
        ...
        <MenuItem
          image={require('../assets/ui/ic_content_burger.png')}
          label="Índice"
          scale={scale}
          onPress={() => {
            onClose();
            setTimeout(onOpenIndex, 180);
          }}
        />
        <MenuItem
          image={require('../assets/ui/ic_font_burger.png')}
        ...
        {showLock && (
          <MenuItem
            image={require('../assets/ui/ic_lock.png')}
            label="Bloquear controles"
            scale={scale}
            onPress={onLock}
            last
          />
...
function MenuItem({
  ...
  return (
    <TouchableOpacity
      style={[
        styles.item,
        { minHeight: 42 * scale, paddingHorizontal: 13 * scale, paddingVertical: 8 * scale },
    ...
    >
      <Image
        source={image}
        style={{ width: 20 * scale, height: 20 * scale, tintColor: '#168FD1' }}
        resizeMode="contain"
      />
