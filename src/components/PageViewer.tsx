...
interface PageViewerProps { pages: BookPage[]; currentPage: number; onPageChange: (pageIndex: number) => void; onPageNavigationStart?: () => void; onFinish: () => void; onBackFromFirstPage?: () => void; coverColor: string; pageTexts?: Map<number, string>; showText: boolean; textSize?: number; locked?: boolean; }
const clamp = (value: number, min: number, max: number) => Math.max(min, Math.min(max, value));

export function PageViewer({ pages, currentPage, onPageChange, onPageNavigationStart, onFinish, onBackFromFirstPage, coverColor, pageTexts, showText, textSize = 14, locked = false }: PageViewerProps) {
  ...
  // The Android reference does not show an unlock prompt. Locking the controls
  // reduces the active book viewport and leaves a dark band below it, which
  // visually raises the text panel and navigation arrows.
  const lockedBottomSpace = locked ? Math.round(height * 0.345) : 0;
  const arrowBottom = locked
    ? clamp(textMinHeight * 0.5 - arrowHeight * 0.5, 34 * uiScale, 76 * uiScale)
    : 24 * uiScale;
  ...
  return <View style={styles.container}>
    <View style={[styles.bookViewport, { bottom: lockedBottomSpace }]}>
...
const styles = StyleSheet.create({
  container: { flex: 1, alignSelf: 'stretch', backgroundColor: Colors.backgroundDark, overflow: 'hidden' },
  bookViewport: { position: 'absolute', top: 0, left: 0, right: 0, overflow: 'hidden' },
