  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View
        ...
        style={[styles.modeMenu, { left: leftX, gap: 14 * uiScale, transform: [{ translateY: menuTranslateY }] }]}
      >
        ...
        <TouchableOpacity style={{ width: buttonWidth, height: buttonHeight, borderRadius: buttonHeight / 2 }} onPress={onClose}>
          <LinearGradient colors={['#28D4EB', '#278BEC']} style={[styles.modeButton, { borderRadius: buttonHeight / 2, paddingHorizontal: 24 * uiScale }]}>
      ...
      >
        <Text style={[styles.sectionTitle, { fontSize: 16 * uiScale }]}>Narraciones profesionales</Text>

        <TouchableOpacity
          style={[styles.narrationCard, { borderColor: coverColor, borderRadius: 11 * uiScale, paddingHorizontal: 14 * uiScale, paddingVertical: 10 * uiScale }]}
          onPress={onSelectProfessional}
          accessibilityLabel={`Escuchar narración de ${narratorName}`}
        >
          <Image source={require('../assets/ui/ic_book_listen.png')} style={[styles.cardIcon, { width: 27 * uiScale, height: 27 * uiScale }]} />
          <View style={styles.cardTextWrap}>
            <Text style={[styles.cardNarrator, { fontSize: 13.5 * uiScale }]}>{narratorName || 'Narrador profesional'}</Text>
            <Text style={[styles.cardChild, { fontSize: 11.5 * uiScale }]}>Para {childName || 'ti'}</Text>
          </View>
          <Image source={require('../assets/ui/ic_play.png')} style={[styles.playIcon, { width: 24 * uiScale, height: 24 * uiScale }]} />
        </TouchableOpacity>

        <Text style={[styles.sectionTitle, { fontSize: 16 * uiScale, marginTop: 12 * uiScale }]}>Narraciones personales</Text>
        <Text style={[styles.emptyText, { fontSize: 12.5 * uiScale }]}>El cuento no tiene tu narración</Text>
      </Animated.View>
    </View>
  );
}
