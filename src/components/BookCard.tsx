...
function BookCardComponent({ book,onPress,onDownloadComplete,onToggleFavorite,onDelete,index=0,cardWidth,selectedBookId,isolationProgress }:BookCardProps){
  ...
  return <Animated.View pointerEvents={selectedBookId?'none':'auto'} style={[styles.cardShadow,{width:cardWidth,height:cardHeight,borderRadius:12*uiScale,opacity:Animated.multiply(fadeAnim,isolationOpacity),transform:[{translateX:translateXAnim},{translateY:Animated.add(translateYAnim,isolationTranslateY)},{scale:Animated.multiply(Animated.multiply(scaleAnim,pressScale),isolationScale)}]}]}>
    <TouchableOpacity ref={cardRef} style={[styles.container,{width:cardWidth,height:cardHeight,borderRadius:12*uiScale,backgroundColor:book.coverColor}]} onPress={handlePress} activeOpacity={isAvailable||isIncluded?0.85:1}>
      ...
      {book.hasVoicework&&<View style={[styles.voiceBadge,{bottom:10*uiScale,right:20*uiScale,width:30*uiScale,height:30*uiScale,borderRadius:15*uiScale}]} accessibilityLabel="Cuento con narración"><Image source={require('../assets/ui/ic_book_listen.png')} style={{width:21*uiScale,height:21*uiScale,tintColor:'#FFFFFF'}} resizeMode="contain"/></View>}
...
const styles=StyleSheet.create({cardShadow:{elevation:9,shadowColor:'#000',shadowOffset:{width:0,height:5},shadowOpacity:0.36,shadowRadius:8},container:{overflow:'hidden'},coverSurface:{position:'absolute',top:0,left:0,bottom:0,overflow:'hidden'},coverImage:{width:'100%',height:'100%'},placeholder:{flex:1,justifyContent:'center',alignItems:'center'},placeholderEmoji:{fontSize:48},titleGradient:{position:'absolute',bottom:0,left:0,right:0,justifyContent:'flex-end'},title:{color:Colors.textWhite,fontFamily:'Montserrat-ExtraBold',textAlign:'center',textShadowColor:'rgba(0,0,0,0.65)',textShadowOffset:{width:0,height:2}},downloadContainer:{position:'absolute',top:0,left:0,right:0,bottom:0},bookEdge:{position:'absolute',top:0,right:0,bottom:0,overflow:'hidden'},edgeHighlightStrong:{position:'absolute',top:0,bottom:0,left:1,width:3,backgroundColor:'rgba(255,255,255,0.48)'},edgeHighlightSoft:{position:'absolute',top:0,bottom:0,left:5,width:2,backgroundColor:'rgba(255,255,255,0.24)'},edgeShadow:{position:'absolute',top:0,bottom:0,right:0,width:3,backgroundColor:'rgba(0,0,0,0.22)'},ribbon:{position:'absolute',tintColor:'#FFFFFF'},menuButton:{position:'absolute',justifyContent:'center',alignItems:'center'},menuButtonText:{color:'#4F5364',fontWeight:'bold'},voiceBadge:{position:'absolute',backgroundColor:'rgba(35,142,235,0.94)',justifyContent:'center',alignItems:'center',elevation:3},readBadge:{position:'absolute',width:22,height:22,borderRadius:11,backgroundColor:Colors.success,justifyContent:'center',alignItems:'center'},readBadgeText:{color:Colors.textWhite,fontSize:12,fontWeight:'bold'},favoriteBadge:{position:'absolute',width:22,height:22,borderRadius:11,backgroundColor:Colors.chipOrange,justifyContent:'center',alignItems:'center'},favoriteText:{color:Colors.textWhite,fontSize:13}});
