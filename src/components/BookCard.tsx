...
function BookCardComponent({
  ...
  const alreadyAnimated = animatedBooks.has(book.id);
  ...
  const fadeAnim = useRef(new Animated.Value(alreadyAnimated ? 1 : 0)).current;
  const scaleAnim = useRef(new Animated.Value(alreadyAnimated ? 1 : 0.94)).current;
  const translateXAnim = useRef(
    new Animated.Value(alreadyAnimated ? 0 : index % 3 === 0 ? -30 : index % 3 === 2 ? 30 : 0)
  ).current;
  const translateYAnim = useRef(
    new Animated.Value(alreadyAnimated ? 0 : index % 3 === 1 ? 22 : 14)
  ).current;
  const pressScale = useRef(new Animated.Value(1)).current;
  useEffect(() => {
    if (animatedBooks.has(book.id)) return;
    ...
    ]).start(() => animatedBooks.add(book.id));
  }, [book.id, fadeAnim, index, scaleAnim, translateXAnim, translateYAnim]);
  const handlePress = () => {
    ...
    >
      ...
      >
        <View style={[styles.coverSurface, { right: edgeWidth }]}>
          ...
          <LinearGradient
            ...
            style={[
              ...
              {
                minHeight: cardWidth * 0.31,
                paddingBottom: 15 * uiScale,
        ...
        <Image
          ...
          style={[
            styles.ribbon,
            { top: -2 * uiScale, right: 18 * uiScale, width: 45 * uiScale, height: 62 * uiScale },
          ]}
          resizeMode="stretch"
        />
        <TouchableOpacity
          style={[
            styles.menuButton,
            { top: 4 * uiScale, right: 29 * uiScale, width: 24 * uiScale, height: 42 * uiScale },
          ]}
          onPress={(event) => {
        ...
        >
          <Text
            style={[styles.menuButtonText, { fontSize: 28 * uiScale, lineHeight: 31 * uiScale }]}
          >
            ⋮
          </Text>
        </TouchableOpacity>
        {isAvailable && book.isRead && (
          <View style={[styles.readBadge, { bottom: 10 * uiScale, left: 8 * uiScale }]}>
            <Text style={styles.readBadgeText}>✓</Text>
          </View>
        )}
        {book.isFavorite && (
          <View style={[styles.favoriteBadge, { top: 7 * uiScale, left: 8 * uiScale }]}>
...
const styles = StyleSheet.create({
  ...
  coverSurface: { position: 'absolute', top: 0, left: 0, bottom: 0, overflow: 'hidden' },
  ...
  bookEdge: { position: 'absolute', top: 0, right: 0, bottom: 0, overflow: 'hidden' },
  edgeHighlightStrong: {
    position: 'absolute',
    top: 0,
    bottom: 0,
