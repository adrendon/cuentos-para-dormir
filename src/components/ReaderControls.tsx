...
export function ReaderControls(props: ReaderControlsProps) {
  ...
  return (
    <Animated.View style={[StyleSheet.absoluteFill, styles.container, props.animatedStyle]} pointerEvents={props.interactive ? 'box-none' : 'none'}>
      <TouchableOpacity
        style={[styles.roundButton, styles.homeButton, { width: homeSize, height: homeSize, borderRadius: homeSize / 2, top: edgeY, left: edgeX }]}
      ...
      {(props.listenMode || props.isNarrating) && (
        <View style={[styles.voiceBar, { top: edgeY, width: voiceWidth, height: voiceHeight, borderRadius: voiceHeight / 2, marginLeft: -voiceWidth / 2, paddingHorizontal: 7 * uiScale }]}>
          <TouchableOpacity
            style={[styles.voiceButton, { width: voiceHeight * 0.72, height: voiceHeight * 0.72, borderRadius: voiceHeight }]}
            onPress={props.isNarrating ? (props.isNarrationPaused ? props.onResumeNarration : props.onPauseNarration) : props.onToggleNarration}
      ...
      <View style={[styles.rightActions, { top: edgeY, right: edgeX, gap: rightGap }]}>
        <TouchableOpacity
          style={[styles.roundButton, { width: buttonSize, height: buttonSize, borderRadius: buttonSize / 2 }]}
          onPress={props.onToggleMusic}
          accessibilityLabel={props.musicEnabled ? 'Silenciar música' : 'Activar música'}
        >
          <Image
            source={props.musicEnabled ? require('../assets/ui/ic_music_on.png') : require('../assets/ui/ic_music_off.png')}
            style={{ width: buttonSize * 0.52, height: buttonSize * 0.52, tintColor: '#168FD1' }}
            resizeMode="contain"
          />
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.menuPill, { width: buttonSize * 0.94, height: 31 * uiScale, borderRadius: 16 * uiScale }]}
...
const styles = StyleSheet.create({
  ...
  roundButton: { backgroundColor: '#F3F4EA', justifyContent: 'center', alignItems: 'center', elevation: 5, shadowColor: '#000', shadowOpacity: 0.26, shadowRadius: 5, shadowOffset: { width: 0, height: 2 } },
  ...
  menuPill: { backgroundColor: '#F3F4EA', justifyContent: 'center', alignItems: 'center', elevation: 4, shadowColor: '#000', shadowOpacity: 0.18, shadowRadius: 4, shadowOffset: { width: 0, height: 2 } },
