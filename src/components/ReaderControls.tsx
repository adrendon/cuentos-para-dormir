...
export function ReaderControls(props: ReaderControlsProps) {
  ...
  // Use height as the primary landscape scale so extra-wide phones expose image
  // instead of enlarging every chrome element.
  ...
  const counterWidth = 66 * uiScale;
  const counterHeight = 38 * uiScale;
  ...
  return (
    <Animated.View style={[StyleSheet.absoluteFill, styles.container, props.animatedStyle]} pointerEvents={props.interactive ? 'box-none' : 'none'}>
      <TouchableOpacity style={[styles.roundButton, styles.homeButton, { width: homeSize, height: homeSize, borderRadius: homeSize / 2, top: edgeY, left: edgeX }]} onPress={props.onHome} accessibilityLabel="Volver a la biblioteca">
        <Image source={require('../assets/ui/ic_home.png')} style={{ width: homeSize * 0.56, height: homeSize * 0.56 }} resizeMode="contain" />
      </TouchableOpacity>

      <View style={[styles.pageCounter, { top: edgeY, right: edgeX, width: counterWidth, height: counterHeight, borderRadius: counterHeight / 2 }]}>
        <Text style={[styles.pageCounterText, { fontSize: 15 * uiScale }]}>{props.currentPage + 1}/{props.totalPages}</Text>
      </View>

      {(props.listenMode || props.isNarrating) && (
        <View style={[styles.voiceBar, { top: edgeY, width: voiceWidth, height: voiceHeight, borderRadius: voiceHeight / 2, marginLeft: -voiceWidth / 2, paddingHorizontal: 7 * uiScale }]}>
          <TouchableOpacity style={[styles.voiceButton, { width: voiceHeight * 0.72, height: voiceHeight * 0.72, borderRadius: voiceHeight }]} onPress={props.isNarrating ? (props.isNarrationPaused ? props.onResumeNarration : props.onPauseNarration) : props.onToggleNarration} accessibilityLabel={props.isNarrationPaused ? 'Continuar narración' : 'Pausar narración'}>
            <Image source={props.isNarrationPaused || !props.isNarrating ? require('../assets/ui/ic_play.png') : require('../assets/ui/ic_pause.png')} style={{ width: voiceHeight * 0.31, height: voiceHeight * 0.31 }} resizeMode="contain" />
          </TouchableOpacity>
          ...
          <Image source={require('../assets/ui/ic_book_listen.png')} style={{ width: 19 * uiScale, height: 19 * uiScale, tintColor: '#FFFFFF' }} resizeMode="contain" />
        </View>
      )}

      <View style={[styles.bottomActions, { right: edgeX, bottom: 14 * uiScale, gap: 8 * uiScale }]}>
        <TouchableOpacity style={[styles.roundButton, { width: buttonSize, height: buttonSize, borderRadius: buttonSize / 2 }]} onPress={props.onToggleMusic} accessibilityLabel={props.musicEnabled ? 'Silenciar música' : 'Activar música'}>
          <Image source={props.musicEnabled ? require('../assets/ui/ic_music_on.png') : require('../assets/ui/ic_music_off.png')} style={{ width: buttonSize * 0.52, height: buttonSize * 0.52, tintColor: '#168FD1' }} resizeMode="contain" />
        </TouchableOpacity>
        <TouchableOpacity style={[styles.roundButton, { width: buttonSize, height: buttonSize, borderRadius: buttonSize / 2 }]} onPress={props.onOpenMenu} accessibilityLabel="Abrir opciones">
          <View style={[styles.hamburger, { width: buttonSize * 0.46, height: buttonSize * 0.34 }]}><View style={styles.hamburgerLine} /><View style={styles.hamburgerLine} /><View style={styles.hamburgerLine} /></View>
        </TouchableOpacity>
      </View>
    </Animated.View>
  );
