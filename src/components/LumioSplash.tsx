...
function Stroke({ d, cx, cy, r, rx, ry, length, delay, duration, kind = 'path' }: StrokeProps) {
  ...
  if (kind === 'circle') return <ACircle {...common} cx={cx} cy={cy} r={r} />;
  if (kind === 'ellipse') return <AEllipse {...common} cx={cx} cy={cy} rx={rx} ry={ry} />;
  return <APath {...common} d={d!} />;
}
  ...
  return (
    <View style={styles.root}>
      <Svg width="100%" height="100%" viewBox="0 0 1080 1080" preserveAspectRatio="xMidYMid meet">
        {items.map((item, index) => <Stroke key={index} {...item} />)}
        <AnimatedDots />
      </Svg>
    </View>
  );
...
function AnimatedDots() {
  ...
  useEffect(() => {
    opacity.value = withDelay(4100, withTiming(1, { duration: 420, easing: Easing.out(Easing.cubic) }));
  }, [opacity]);
  ...
  return (
    <>
      <ACircle animatedProps={animatedProps} cx={742} cy={385} r={10} fill={WHITE} />
      <ACircle animatedProps={animatedProps} cx={810} cy={360} r={7} fill={WHITE} />
    </>
  );
}
