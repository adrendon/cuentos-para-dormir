...
export default function BookScreen() {
  ...
  const contentGender =
    mode === 'listen' && voiceworkProfile ? voiceworkProfile.gender : profile.gender;
  ...
  const handleSelectIndexPage = useCallback(
    (pageIndex: number) => {
      setShowIndex(false);
  ...
  const handleTapScreen = useCallback(() => {
    if (!readerLock.isLocked && !showMenu && !showIndex) setShowControls((prev) => !prev);
  }, [readerLock.isLocked, showMenu, showIndex]);
  const handleBackFromFirstPage = useCallback(() => {
    ...
    setStage('intro');
    setShowControls(true);
  }, [stopNarration, setCurrentPage, readerLock.unlock]);
  ...
  const handleReadAgain = useCallback(() => {
    ...
    setShowControls(true);
    setStage('reading');
  }, [setCurrentPage, readerLock.unlock]);
  const handleShare = useCallback(async () => {
    try {
      await Share.share({ message: `¡Te recomiendo el cuento "${title || book?.title}"!` });
    } catch (error) {
    ...
    <Animated.View style={[styles.container, screenAnimStyle]}>
      ...
      {stage === 'intro' ? (
        ...
        <NarrationPanel
          ...
          musicEnabled={bookMusic.isPlaying}
          onToggleMusic={() => {
            void bookMusic.toggle();
          }}
          onHome={() => {
            void handleReturnToModeMenu();
          }}
        ...
        <>
          {!showEndScreen ? (
            ...
            <BookEndScreen
              ...
              narrator={voiceworkProfile?.narrator}
              backgroundUri={pages[pages.length - 1]?.uri}
          ...
          {!showEndScreen && !readerLock.isLocked && (
            <ReaderControls
              ...
              onMusicVolumeChange={(volume) => {
                void bookMusic.changeVolume(volume);
              }}
            />
          )}
          <ReaderMenu
          ...
          />
          {mode === 'listen' && readerLock.isLocked && !showEndScreen && (
            <LockOverlay
              showPrompt={readerLock.showUnlockPrompt}
              onRequestPrompt={readerLock.requestUnlock}
              onDismissPrompt={readerLock.hidePrompt}
              onUnlock={() => {
                readerLock.unlock();
                setShowControls(true);
              }}
